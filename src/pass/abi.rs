...
use crate::{func::Func, isn::Isn, target::Target, ty::TypeQueries, unit::Unit, val::Val};

pub struct Pass;

struct FuncPass<'a> {
    uniq_index_per_func: u32,
    target: &'a Target,
}

impl super::Pass for Pass {
    fn run(&mut self, unit: &mut Unit) {
        for func in unit.globals.funcs() {
            let mut func_pass = FuncPass {
                // stret_stash: None; // TODO
                uniq_index_per_func: 0,
                target: unit.target,
            };
...
impl FuncPass<'_> {
    fn run(&mut self, func: &mut Func) {
        self.convert_returns(func);
    }

    fn convert_returns(&mut self, func: &mut Func) {
        let retty = func.ty().called().unwrap();

        for blk in func.blocks() {
	    let mut to_insert = vec![];

            for (i, isn) in blk.isns().iter().enumerate() {
                if let Isn::Ret(ref retval) = isn {
                    if retty.is_struct() {
                        todo!("return <struct>")
                    } else if retty.is_float() {
                        todo!("fpret")
                    } else if retty.is_void() {
                        // nothing
                    } else {
                        assert!(retty.is_integral() || retty.deref().is_some());

                        let abiv = Val::new_reg(self.target.abi.ret_regs.int(0), retty);
                        let movret = Isn::Copy {
                            from: Rc::clone(retval),
                            to: Rc::new(abiv),
                        };

			to_insert.push((i, movret));
                    }
                }
            }

	    let mut isns = blk.isns_mut();
	    for (i, isn) in to_insert {
		isns.insert(i, isn);
	    }
        }
    }
